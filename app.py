...
def index():
    ...
    if (
        not code
        or scope
        != "channel:read:subscriptions moderation:read channel:manage:broadcast channel:manage:polls channel:manage:predictions channel:read:polls channel:read:predictions channel:read:vips channel:manage:vips"
    ):
        error = "неверные разрешения"

    token_data = requests.post(
        f'https://id.twitch.tv/oauth2/token?client_id={os.getenv("CLIENT_ID")}&client_secret={os.getenv("CLIENT_SECRET")}&code={code}&grant_type=authorization_code&redirect_uri=http://localhost:5000/api/v1/auth'
    ).json()
